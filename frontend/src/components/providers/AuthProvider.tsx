 ...
 *
 * This means: if the user has a valid refresh token cookie, they'll be
 * re-authenticated transparently on every page load without needing to log in.
 */
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  // Initialize CSRF token cookie on mount
  useInitCsrf()
  ...
  useEffect(() => {
    // If we got user data back, the silent refresh worked
    // The api interceptor already set the access token in Zustand,
    // but we set the user here too to make sure it's in sync
    if (user) {
